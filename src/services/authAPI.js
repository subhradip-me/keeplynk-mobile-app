...
export const authAPI = {
  ...
  login: async (credentials) => {
    try {
      ...
      if (response.success) {
        // Store token and user in memoryStorage (React Native equivalent of localStorage)
        await memoryStorage.setItem('auth_token', response.data.token);
        await memoryStorage.setItem('user_data', JSON.stringify(response.data.user));
  ...
  register: async (userInfo) => {
    try {
      ...
      if (response.success) {
        // Store token and user in memoryStorage
        await memoryStorage.setItem('auth_token', response.data.token);
        await memoryStorage.setItem('user_data', JSON.stringify(response.data.user));
        apiService.setToken(response.data.token);
        return response.data;
